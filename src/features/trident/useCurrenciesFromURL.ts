...
import { USDC } from 'app/config/tokens/rails'
...
const useCurrenciesFromURL = (): {
  ...
  const currencyB =
  //@ts-ignore
    useCurrency(getToken(router.query.tokens?.[1], chainId)) || (chainId && ChainId == 1 ? BSP1 : chainId == 137 ? MEMELON : chainId==56 ? BSP:USDC) || undefined
